...
REQUIREMENTS (
    Сompiler : MinGW-w64 (w64devkit: https://github.com/skeeto/w64devkit/releases)
...
DEC_FUNC(HMODULE) GetNtdllAddr(CONST_PTR(MN_PEB) peb) {
    ...
    FOR_LIKE (const LIST_ENTRY *e = mdl->Flink,  e != mdl,  e = e->Flink) {
        ...
        IF_UNLIKE (rc->BaseDllName.Length == NTDLL_NAME_SZ) {
            ...
            IF_LIKE ((nm | ASCII_CASE_MASK) == NTDLL_NAME_HASH)
    ...
    WHILE_LIKE (rl < rlEnd) {
        ...
        FOR_LIKE (,  it < itEnd,  it++) {
            ...
            IF_LIKE (RELOC_IS_DIR64(entry)) {
                const DWORD ofs = RELOC_BLOCK_OFFSET(entry);
...
DEC_FUNC(const BYTE*) ReadOverLay(VOID) {
    ...
    *(PUINT32)dtBuf = tlLn;
    ...
    IF_NTFAIL (pNtReadFile(
        ...
        NULL
    )) FreeBuf(&dtBuf); // dtBuf is assigned NULL
...
DEC_FUNC(BOOLEAN) AntiDebugStage(
    ...
    IF_LIKE (stage == ADB_STAGE_PREAPI) {
        return *(volatile const BOOLEAN*)KUSER_KD_DEBUGGER_ENABLED
               || peb->BeingDebugged
